...
| `FoldDangerZoneComponent`                                | `fold-danger-zone`                     | Framed destructive-action block for “delete X” panels/forms: alert-toned region, a `title`, a projected blast-radius explanation, and an optional **type-to-confirm** guard (`confirmPhrase`, trimmed/case-sensitive). The destructive control is projected via `[actions]`; the consumer wires its `[disabled]` to the two-way `armed` model (always `true` when no phrase). `role="group"` + `aria-labelledby`. Composes inside `fold-panel-host`.                                                                                                                                                                                                                                                                                                        |
