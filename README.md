...
| ------------------------------------------------------- | ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `Sh3AppShellComponent`                                  | `sh3-app-shell`     | Responsive app skeleton (rails + header + content + self-collapsing `footer` slots; `headerLayout`/`footerLayout` inset·full, `footerBehavior` pinned·scroll; `mobileNav` drawer·none — `[(mobileNavOpen)]` off-canvas drawer for the primary rail on mobile, or `none` to compose an `sh3-nav-launcher`; `contentScroll` clip·auto; built-in skip-link to a focusable `<main>`. Regions float per-surface via `sh3Elevated`, not a shell flag). |
...
Directives worth knowing: **`sh3Surface`** (`page`·`chrome` — the seam a mixed
theme re-colours across) and **`sh3Elevated`** (raise any bg-owning element into
an inset, rounded, shadowed card — the per-surface "floating" mechanism, driven
by `--sh3-surface-inset`/`-radius`/`-shadow`).
