...
| `FoldCardComponent`                                      | `fold-card`                            | Raised content surface (`surface-card`/`sunken`, hairline border, consistent radius). Optional projected `[cardHeader]`/`[cardFooter]` bands with per-band chrome (`separators`/`raisedBands` = `none`/`header`/`footer`/`both`); the body padding never shifts when a band toggles. `interactive` makes the whole card an accessible button (`role`/`tabindex`, focus ring, Enter/Space/click → `(activated)`).                                   |
