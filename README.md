...
| `Sh3InputComponent`                                     | `sh3-input`         | Text-input control (`value: string`) — Signal Forms (`[formField]`) or standalone `[(value)]`; size × align × variant, optional label. The _edit_ half of a record (`sh3-field` reads). |
| `Sh3NumberInputComponent`                               | `sh3-number-input`  | Numeric sibling of `sh3-input` (`value: number \| null`, empty ⇒ `null`); owns `min` / `max` / `step`. Split so each control keeps its true type.                                       |
