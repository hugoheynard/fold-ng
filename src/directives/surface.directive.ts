 ...
 *   --fold-color-text: var(--fold-ref-slate-50); / * light text on the dark rail * /
 * }
 * ```
 *
 ...
 * - `[data-surface] { color: var(--fold-color-text); }` — every surface resolves
 ...
 *
 * This replaces reaching into `fold-app-shell`'s internal class names from the
 ...
 *
 * @selector `[foldSurface]`
 *
 * @example
 * ```html
 * <aside foldSurface="chrome">…dark furniture on a light page…</aside>
 * ```
 */
@Directive({
  selector: "[foldSurface]",
  standalone: true,
  host: { "[attr.data-surface]": "foldSurface()" },
})
export class FoldSurfaceDirective {
  /** The surface this element is. Defaults to `page`, including a bare
   *  `foldSurface` attribute (which the template binds as an empty string). */
  readonly foldSurface = input<FoldSurfaceName, FoldSurfaceName | "">("page", {
