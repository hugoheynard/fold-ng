...
/** Consumer icons merged in at bootstrap — set via {@link provideIcons}. */
export const ICON_OVERRIDES = new InjectionToken<IconSet>(
  "sh3.icons.overrides",
);
 ...
 * ```
 */
export function provideIcons(icons: IconSet): Provider {
 ...
 * {@link provideIcons} (bootstrap) or {@link register}/{@link registerMany}
 * (runtime). `sh3-icon` resolves every `name` through here.
 */
@Service()
export class IconRegistry {
  private readonly _icons = signal<IconSet>({
    ...SH3_BUILTIN_ICONS,
  ...
  registerMany(icons: IconSet): void {
