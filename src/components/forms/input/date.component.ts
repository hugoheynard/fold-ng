...
/** The native temporal input types `fold-date` wraps — one control, because they
 *  share the exact chrome and a `string` value; only the picker differs. */
export type FoldDateType =
  "date" | "time" | "datetime-local" | "month" | "week";

/**
 * `<fold-date>` — the temporal-field sibling of {@link FoldInputComponent}: a thin
 * wrapper around a **native `<input type="date">`** (and its family — `time`,
 * `datetime-local`, `month`, `week` via {@link type}), like `fold-select` wraps the
 ...
 * string: `YYYY-MM-DD` for a date, `HH:mm` for a time) — so consumers stop
 * hand-writing an `inputValue($event)` reader on a bare `<input type="date">`.
 *
 * The native pickers keep the OS calendar / clock + mobile keyboard for free (same
 ...
 *
 * <!-- A time field, standalone two-way -->
 * <fold-date type="time" [(value)]="pickupTime" />
 * ```
 */
@Component({
  selector: "fold-date",
