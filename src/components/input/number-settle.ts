/**
 * Pure numeric-settling helpers for {@link FoldNumberInputComponent}.
 *
...
function snapToGrid(n: number, c: FoldNumberConstraints): number {
  const base = c.min ?? 0;
  return base + Math.round((n - base) / c.step) * c.step;
}

/** Round to the effective precision (kills float drift like 0.1 + 0.2). */
function round(n: number, c: FoldNumberConstraints): number {
...
function clamp(n: number, c: FoldNumberConstraints): number {
  let out = n;
  if (c.min !== undefined) {
 ...
 */
export function settleNumber(n: number, c: FoldNumberConstraints): number {
 ...
 */
export function isStepAligned(c: FoldNumberConstraints): boolean {
  if (c.max === undefined) {
    return true;
  }
