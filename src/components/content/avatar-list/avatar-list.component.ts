...
/** One face in an {@link FoldAvatarListComponent} — the minimum an avatar needs,
...
export interface FoldAvatarListItem {
...
/**
 * `<fold-avatar-list>` — a row of slightly-overlapping avatars (the org-chart
 ...
 * override `--fold-avatar-list-ring` when the list sits on a non-card surface.
 *
 * ## Inputs
 *
 * | Input     | Type                    | Default   | Description                                              |
 * |-----------|-------------------------|-----------|----------------------------------------------------------|
 * | `avatars` | `FoldAvatarListItem[]` (required) | — | The faces, in order.                                     |
 ...
 * `FoldAvatarListItem`, since they vary member-by-member (a guest among members,
 * one person absent, another arriving).
 *
 * @selector `fold-avatar-list`
 *
 * @example
 * ```html
 * <fold-avatar-list [avatars]="occupants" [limit]="4" top="first" />
 * ```
 */
@Component({
  selector: "fold-avatar-list",
...
export class FoldAvatarListComponent {
  readonly avatars = input.required<readonly FoldAvatarListItem[]>();
