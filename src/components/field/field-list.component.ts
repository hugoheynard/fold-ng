...
/**
 * `<sh3-field-list>` — the read-only counterpart of a form: a description list
 * (`<dl>`) of label/value pairs. It's what fills the *display* side of a detail
 * page (the recap), where `sh3-input` fills the *edit* side. Nothing here is
 * interactive — it renders values you already know, not fields you type into.
 *
 * Put {@link Sh3FieldComponent} children inside it:
 *
 * ```html
 * <sh3-field-list>
 *   <sh3-field label="Contract type">CDI</sh3-field>
 *   <sh3-field label="Job title" [empty]="!c.jobTitle">{{ c.jobTitle }}</sh3-field>
 *   <sh3-field label="Compensation">
 ...
 * has to match them. The `dt`/`dd` are contributed by each `sh3-field`
 ...
 * `--sh3-field-list-label-width` (160px — the fixed label column) ·
 * `--sh3-field-list-col-gap` (16px, label→value) ·
 * `--sh3-field-list-row-gap` (10px, between rows). Shrink the label width when
 * the list sits in a narrow rail.
 *
 * @selector `sh3-field-list`
 */
@Component({
  selector: "sh3-field-list",
  standalone: true,
...
export class Sh3FieldListComponent {}
