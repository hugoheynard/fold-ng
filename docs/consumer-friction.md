...
| `fold-search`                        | `value` model **+** `searchChange`                    | 📌 **kept** — `value` is the term as STATE (every keystroke), `searchChange` the same term as an EVENT once it settles. Deux questions, pas deux façons de lire l'une | 📌     |
